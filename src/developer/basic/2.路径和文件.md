...
main.js 文件在 `/user/linuxno/my-project/main.js`
tools.js 文件在 `/user/linuxno/my-project/tools.js`

绝对路径引用:

```js title="main.js"
import tools from '/user/linuxno/my-project/tools.js';
```
...
此时，这两种引用方式都是正确的，代码都可以顺利运行。因为两个文件都在 `linuxno/my-project` 下。

这个时候，如果另一个人下载了项目，他的工作目录不是 `linuxno` ， 是 `ccc`，那么文件的的路径就变成了:

main.js 文件会变成 `/user/ccc/my-project/main.js`
tools.js 文件会变成 `/user/ccc/my-project/tools.js`

那么绝对路径引用就必须得修改成下面这样:

```js title="main.js"
import tools from '/user/ccc/my-project/tools.js';
```
...
windows 下一般定位资源必须得带上盘符，比如 `c:\user\linuxno\image`
Linux 系统则是 `/home/linuxno/image`
...
你自己本地的一个文件例如 `c:\user\linuxno\my-project\index.html` 要怎么才能共享给别人，让别人都能访问到呢？不太可能把电脑搬到人家里去吧？
...
`c:\user\linuxno\my-project\index.html`
...
当前用户为一般用户且用户名为 `linuxno` 用户，则 `~/` 等价于 `/home/linuxno/`
